...
export default function PipelineStatus() {
  const { getActiveExecutionsList, setActiveExecution, removeActiveExecution } = usePipelineStore();
  ...
  useEffect(() => {
      ...
      try {
        ...
        for (const execution of activeExecutions) {
          if (execution.overallStatus === "running" || execution.overallStatus === "paused") {
            try {
              const response: PipelineStatusResponse = await pipelineApi.getStatus(execution.executionId);
              
              // 완료된 경우 활성 목록에서 제거
              if (response.data.overallStatus === "completed" || response.data.overallStatus === "failed") {
                removeActiveExecution(execution.executionId);
              } else {
                setActiveExecution(execution.executionId, response.data);
              }
            } catch (error) {
    ...
    // 활성 파이프라인이 있을 때만 폴링 시작
    if (activeExecutions.length > 0) {
      fetchActiveStatuses(); // 즉시 한 번 실행
      const interval = setInterval(fetchActiveStatuses, 5000); // 5초마다 상태 확인
    ...
    <div className="bg-white rounded-xl p-6 shadow-md border border-gray-200">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-800 mb-1">파이프라인 실행 상태</h3>
      ...
      {activeExecutions.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-gray-500 text-sm">현재 실행 중인 파이프라인이 없습니다</p>
          <p className="text-gray-400 text-xs mt-1">스케줄에 따라 자동 실행되거나 수동으로 실행할 수 있습니다</p>
        </div>
      ) : (
            ...
            <div key={execution.executionId} className="border rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center">
                  ...
                  <span className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(execution.overallStatus)}`}>
              ...
              <div className="text-xs text-gray-400 mt-1">
                시작 시간: {new Date(execution.startedAt).toLocaleString('ko-KR')}
              </div>
            </div>
          ))}
        </div>
