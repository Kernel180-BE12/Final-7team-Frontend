...
export default function Sidebar() {
  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      try {
        ...
        if (response.success) {
          // 성공 시 API 데이터 사용
          const sortedItems = response.data.sort((a, b) => a.orderSeq - b.orderSeq); // 순서대로 정렬
          setMenuItems(sortedItems.filter(item => item.isActive)); // 활성화된 메뉴만 필터링
        } else {
          setError('sidebar', response.message || "메뉴를 불러오는데 실패했습니다."); // API 실패 에러 처리
  ...
  return (
    <nav className="w-70 bg-gray-800 p-8 shadow-2xl"> {/* 사이드바 네비게이션 */}
      {/* 로고 영역 */}
      ...
      <ul className="space-y-3">
        {menuItems.map((item) => ( // 동적 메뉴 아이템 렌더링
          <li key={item.id}>
            <button
            ...
            >
              ...
              {item.label}
              {isLoading.sidebar && item === menuItems[0] && ( // 첫 번째 메뉴에만 로딩 표시
